# Versão Final e Completa - v4.5.1
...
if not st.session_state.get("authenticated", False):
    ...
    with tab2:
        ...
        if "membros" in st.session_state and st.session_state.membros:
            ...
            for index, membro in df_membros_tab2.iterrows():
                with st.container(border=True):
                    ...
                    with col_selecao:
                        ...
                        if selecionado: membros_selecionados_chaves.add((membro.get('Nome'), membro.get('Data de Nascimento')))
                    with col_info:
            ...
            with col1_act:
                if st.button("🟢 Marcar como Ativos", use_container_width=True, disabled=sem_selecao):
                    st.session_state.chaves_para_status = membros_selecionados_chaves; st.session_state.novo_status = "ATIVO"; st.session_state.confirmando_status = True; st.rerun()
            with col2_act:
                if st.button("🔴 Marcar como Inativos", use_container_width=True, disabled=sem_selecao):
                    st.session_state.chaves_para_status = membros_selecionados_chaves; st.session_state.novo_status = "INATIVO"; st.session_state.confirmando_status = True; st.rerun()
            with col3_act:
                if st.button("🔄 Recarregar Dados"): st.session_state.membros = carregar_membros(); st.rerun()
        else:
    ...
    with tab3:
        ...
        if df_original.empty: st.warning("Não há membros cadastrados para exibir.")
            ...
            if not termo and not data_filtro:
                ...
                for index, membro in df_filtrado.iterrows():
                    with st.container(border=True):
                        ...
                        with col_selecao_b:
                            ...
                            if selecionado: chaves_selecionadas_excluir.add((membro.get('Nome'), membro.get('Data de Nascimento')))
                        with col_info_b:
                            status_icon = '🟢' if str(membro.get('Status')).upper() == 'ATIVO' else '🔴' if str(membro.get('Status')).upper() == 'INATIVO' else '⚪'
                            st.subheader(f"{status_icon} {membro.get('Nome')}")
                ...
                if st.session_state.get('confirmando_exclusao', False):
                    ...
                    with col1_del:
                        if st.button("🗑️ Excluir Registros Selecionados", use_container_width=True, disabled=sem_selecao):
                            st.session_state.chaves_para_excluir = chaves_selecionadas_excluir; st.session_state.confirmando_exclusao = True; st.rerun()
                    with col2_del:
                        df_para_exportar = df_original[df_original.apply(lambda row: (row['Nome'], row['Data de Nascimento']) in chaves_selecionadas_excluir, axis=1)].reindex(columns=HEADERS)
                        ...
                        st.download_button("📄 Exportar Excel (.xlsx)", excel_data, "membros_selecionados.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, disabled=sem_selecao)
                    with col3_del:
                        df_para_exportar = df_original[df_original.apply(lambda row: (row['Nome'], row['Data de Nascimento']) in chaves_selecionadas_excluir, axis=1)].reindex(columns=HEADERS)
                        pdf_data = criar_pdf_lista(df_para_exportar)
                        st.download_button("📕 Exportar PDF (.pdf)", pdf_data, "membros_selecionados.pdf", "application/pdf", use_container_width=True, disabled=sem_selecao)
