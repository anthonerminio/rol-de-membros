...
def criar_pdf_exportacao_busca(df):
    ...
    for _, row in df.iterrows():
        ...
        write_field("Data de Nascimento", row['Data de Nascimento']); write_field("Telefone", row['Celular']); write_field("Forma de Admissão", row['Forma de Admissao']); write_field("Data de Admissão", row['Data de Admissao']); write_field("Data de Conversão", row['Data de Conversao'])
        pdf.ln(5); pdf.line(pdf.get_x(), pdf.get_y(), pdf.w - pdf.r_margin, pdf.get_y()); pdf.ln(5)
...
def criar_pdf_aniversariantes_com_status(ativos_df, inativos_df, outros_df, mes_nome):
    ...
    def draw_section(title, df_section):
        if not df_section.empty:
            ...
            pdf.ln(8)
    draw_section("🟢 Aniversariantes Ativos", ativos_df); draw_section("🔴 Aniversariantes Inativos", inativos_df); draw_section("⚪ Aniversariantes com Status Não Definido", outros_df)
    return bytes(pdf.output())
...
def criar_pdf_ficha(membro):
    ...
    draw_section_header("⛪ Dados Eclesiásticos"); draw_field("Status", membro.get("Status")); draw_field("Forma de Admissão", membro.get("Forma de Admissao")); draw_field("Data de Admissão", membro.get("Data de Admissao")); draw_field("Data de Conversão", membro.get("Data de Conversao")); pdf.ln(5)
    if membro.get("Observações") and str(membro.get("Observações")).strip():
        draw_section_header("📝 Observações"); draw_field("", membro.get("Observações"))
    return bytes(pdf.output())
...
def init_state():
    ...
    if st.session_state.authenticated:
        ...
        if "confirmando_exclusao" not in st.session_state: st.session_state.confirmando_exclusao, st.session_state.chaves_para_excluir = False, set()
        if "confirmando_status" not in st.session_state: st.session_state.confirmando_status, st.session_state.chaves_para_status, st.session_state.novo_status, st.session_state.obs_status = False, set(), "", ""
        if "selecao_lista" not in st.session_state: st.session_state.selecao_lista = set()
        if "selecao_busca" not in st.session_state: st.session_state.selecao_busca = set()
        if "editing_member_key" not in st.session_state: st.session_state.editing_member_key = None
        if "editing_member_index" not in st.session_state: st.session_state.editing_member_index = None
        for key in MAP_KEYS.values():
...
if not st.session_state.get("authenticated", False):
    ...
    with col_reload:
        if st.button("🔄 Sincronizar Dados", use_container_width=True):
            st.session_state.membros = carregar_membros(); st.toast("Dados sincronizados com sucesso!")
    with col_logout:
    ...
    with tab3:
        ...
        if df_original.empty: st.warning("Não há membros cadastrados para exibir.")
            ...
            if termo: df_filtrado = df_filtrado[df_filtrado.apply(lambda row: termo in str(row.get('Nome', '')).upper() or termo in str(row.get('CPF', '')), axis=1)]
            ...
            if not df_para_exportar.empty:
                output_excel = BytesIO(); df_para_exportar.to_excel(output_excel, index=False, sheet_name='Membros', engine='openpyxl'); excel_data = output_excel.getvalue()
                pdf_data = criar_pdf_exportacao_busca(df_para_exportar)
            col_excel, col_pdf = st.columns(2)
            with col_excel: st.download_button("📄 Exportar Excel", excel_data, "exportacao_membros.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, disabled=sem_selecao_busca)
            with col_pdf: st.download_button("📕 Exportar PDF", pdf_data, "exportacao_membros.pdf", "application/pdf", use_container_width=True, disabled=sem_selecao_busca)
            st.markdown("---")
            if df_filtrado.empty and (termo or data_filtro): st.warning("Nenhum membro encontrado com os critérios de busca especificados.")
            else:
    ...
    with tab4:
        ...
        if "membros" in st.session_state and st.session_state.membros:
            ...
            if mes_selecionado:
                aniversariantes_df = df_membros[df_membros['Mês'] == meses_pt[mes_selecionado]].sort_values(by='Dia')
                if aniversariantes_df.empty: st.info("Nenhum aniversariante encontrado para este mês.")
                    ...
                    ativos_df = aniversariantes_df[aniversariantes_df['Status'].str.upper() == 'ATIVO']; inativos_df = aniversariantes_df[aniversariantes_df['Status'].str.upper() == 'INATIVO']; outros_df = aniversariantes_df[~aniversariantes_df['Status'].str.upper().isin(['ATIVO', 'INATIVO'])]
                    ...
                    def display_birthday_section(title, df_section, icon):
                        if not df_section.empty:
                            ...
                            for _, row in df_section.iterrows():
                                with st.container(border=True): st.markdown(f"**Dia {row['Dia']}** - {row['Nome']}")
                            st.markdown("<br>", unsafe_allow_html=True)
                    display_birthday_section("Aniversariantes Ativos", ativos_df, "🟢"); display_birthday_section("Aniversariantes Inativos", inativos_df, "🔴"); display_birthday_section("Aniversariantes com Status Não Definido", outros_df, "⚪")
                    st.markdown("---")
                    pdf_data = criar_pdf_aniversariantes_com_status(ativos_df.rename(columns=df_display_cols), inativos_df.rename(columns=df_display_cols), outros_df.rename(columns=df_display_cols), mes_selecionado)
        ...
        else: st.info("Não há membros cadastrados para gerar a lista de aniversariantes.")
    ...
    with tab5:
        ...
        with col_filtro2: data_nasc_range = st.date_input("Filtrar por Data de Nascimento", value=(), key="edit_dob_range", min_value=date(1910, 1, 1), max_value=date(2030, 12, 31), format="DD/MM/YYYY")
        with col_filtro3: data_adm_range = st.date_input("Filtrar por Data de Admissão", value=(), key="edit_adm_range", min_value=date(1910, 1, 1), max_value=date(2030, 12, 31), format="DD/MM/YYYY")
        df_membros_edicao = pd.DataFrame(st.session_state.membros)
        if not df_membros_edicao.empty:
            if termo_busca_edicao: df_membros_edicao = df_membros_edicao[df_membros_edicao.apply(lambda row: termo_busca_edicao in str(row.get('Nome', '')).upper() or termo_busca_edicao in str(row.get('CPF', '')), axis=1)]
            if len(data_nasc_range) == 2: df_membros_edicao['Data de Nascimento_dt'] = pd.to_datetime(df_membros_edicao['Data de Nascimento'], format='%d/%m/%Y', errors='coerce'); df_membros_edicao = df_membros_edicao.dropna(subset=['Data de Nascimento_dt']); df_membros_edicao = df_membros_edicao[(df_membros_edicao['Data de Nascimento_dt'].dt.date >= data_nasc_range[0]) & (df_membros_edicao['Data de Nascimento_dt'].dt.date <= data_nasc_range[1])]
            if len(data_adm_range) == 2: df_membros_edicao['Data de Admissao_dt'] = pd.to_datetime(df_membros_edicao['Data de Admissao'], format='%d/%m/%Y', errors='coerce'); df_membros_edicao = df_membros_edicao.dropna(subset=['Data de Admissao_dt']); df_membros_edicao = df_membros_edicao[(df_membros_edicao['Data de Admissao_dt'].dt.date >= data_adm_range[0]) & (df_membros_edicao['Data de Admissao_dt'].dt.date <= data_adm_range[1])]
            st.divider()
            ...
            for index, membro in df_membros_edicao.iterrows():
                with st.container(border=True):
                    ...
                    with col_pdf_col:
                        ...
                        st.download_button(label="🖨️ PDF", data=pdf_data_ficha, file_name=f"ficha_{membro.get('Nome').replace(' ', '_').lower()}.pdf", mime="application/pdf", key=f"pdf_btn_{index}")
                    if st.session_state.editing_member_key == index:
                        st.session_state.editing_member_index = index; membro_para_editar = membro
                        st.divider()
                        with st.form(key=f"edit_form_{index}"):
                            ...
                            except (ValueError, TypeError): data_nasc_obj, data_conv_obj, data_adm_obj = None, None, None
                            c1, c2 = st.columns(2)
                            ...
                            st.subheader("Informações Eclesiásticas e Adicionais"); c9, c10 = st.columns(2)
                            with c9: st.selectbox("Forma de Admissão", forma_admissao_options, index=get_safe_index(forma_admissao_options, membro_para_editar.get("Forma de Admissao")), key="edit_forma_admissao"); st.selectbox("Status", status_options, index=get_safe_index(status_options, membro_para_editar.get("Status", "Ativo")), key="edit_status"); st.date_input("Data de Conversão", value=data_conv_obj, key="edit_data_conv", format="DD/MM/YYYY"); st.date_input("Data de Admissão", value=data_adm_obj, key="edit_data_adm", format="DD/MM/YYYY")
                            with c10: st.selectbox("Grau de Instrução", grau_instrucao_options, index=get_safe_index(grau_instrucao_options, membro_para_editar.get("Grau de Instrução")), key="edit_grau_ins"); st.text_area("Observações", value=membro_para_editar.get("Observações"), key="edit_observacoes", height=155)
                            st.divider()
