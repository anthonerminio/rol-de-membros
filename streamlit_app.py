...
try:
    ...
    GOOGLE_REDIRECT_URI = "https://pibgaibu.streamlit.app" # URL de produção
...
def criar_pdf_exportacao_busca(df):
    ...
    for _, row in df.iterrows():
        ...
        write_field("Data de Nascimento", row['Data de Nascimento'])
        write_field("Telefone", row['Celular'])
        write_field("Forma de Admissão", row['Forma de Admissao'])
        write_field("Data de Admissão", row['Data de Admissao'])
        write_field("Data de Conversão", row['Data de Conversao'])
...
def criar_pdf_ficha(membro):
    ...
    set_font_and_write('B', 16, 'Ficha Individual de Membro - PIB Gaibu')
    set_font_and_write('', 14, membro.get("Nome", ""))
    pdf.ln(5)
...
# --- Funções de Dados (Google Sheets) e Callbacks ---
...
def limpar_formulario():
    for key in MAP_KEYS.values():
        ...
        elif key == 'sexo': st.session_state[key] = "M"
...
if not st.session_state.get("authenticated", False):
    ...
    with tab4: # ... (sem alterações)
        ...
        if "membros" in st.session_state and st.session_state.membros:
            ...
            if 'Data de Nascimento' in df_membros.columns and not df_membros['Data de Nascimento'].isnull().all():
                ...
                st.info("Não há membros com data de nascimento cadastrada para gerar a lista.")
        else:
            ...
            for index, membro in df_membros_edicao.iterrows():
                with st.container(border=True):
                    ...
                    with col_pdf:
                        ...
                        st.download_button(label="🖨️ PDF", data=pdf_data_ficha, file_name=f"ficha_{membro.get('Nome', 'membro').replace(' ', '_').lower()}.pdf", mime="application/pdf", key=f"pdf_btn_{index}")
