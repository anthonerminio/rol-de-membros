    ...
    with tab1:
        ...
        with st.form("form_membro", clear_on_submit=True):
            ...
            if st.form_submit_button("💾 Salvar Membro"):
                novo = {}
                # Mapeamento explícito de chaves de formulário para cabeçalhos
                map_keys = {
                    ...
                    "Cônjuge": "conjuge", "CEP": "cep", "Endereco": "endereco", "Bairro": "bairro", "Cidade": "cidade",
                    "UF (Endereco)": "uf_end", "Grau de Instrucao": "grau_ins", "Celular": "celular",
                    "Data de Conversao": "data_conv", "Data de Admissao": "data_adm", "Status": "status", "Observações": "observacoes"
                }
                for header, key in map_keys.items():
                    valor = st.session_state.get(key, "")
                    if isinstance(valor, date):
                        novo[header] = valor.strftime('%d/%m/%Y')
                    elif isinstance(valor, str):
                        novo[header] = valor.strip().upper()
                    else:
                        novo[header] = valor
